...
function Consumer() {
  const { selectedCrypto, selectedPeriod, setSelectedPeriod, cryptoHistory } =
...
describe("CryptoContext", () => {
  ...
  it("deve reutilizar historico em cache quando voltar para o periodo anterior", async () => {
    const user = userEvent.setup();

    const marketsPayload = [{ id: "bitcoin", name: "Bitcoin", symbol: "btc" }];
    ...
    await waitFor(() =>
      expect(screen.getByTestId("selected-id")).toHaveTextContent("bitcoin")
    );
    ...
    await waitFor(() =>
      expect(screen.getByTestId("history-size")).toHaveTextContent("2")
    );
